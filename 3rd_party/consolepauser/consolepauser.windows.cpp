...
string GetErrorMessage() {
  ...
  FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                 NULL, GetLastError(),
                 MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), &result[0],
                 result.size(), NULL);
...
void PauseExit(int exitcode, bool reInp) {
  ...
  if (reInp) {
    ...
    HANDLE hInp =
        CreateFileA("CONIN$", GENERIC_WRITE | GENERIC_READ, FILE_SHARE_READ,
                    &sa, OPEN_EXISTING, /*FILE_ATTRIBUTE_NORMAL*/ 0, NULL);
    // si.hStdInput = hInp;
...
int main(int argc, char **argv) {
  ...
  if (argc < 3) {
    ...
    printf("\nUsage: consolepauser.exe <0|1> <filename> "
           "<parameters>\n");
    printf("\n 1 means the STDIN is redirected by algorimejo; 0 means not\n");
  ...
  WINBOOL bSuccess = SetInformationJobObject(
  ...
  string command =
      GetCommand(argc, argv, reInp, pauseAfterExit, enableVisualTerminalSeq);
  HANDLE hOutput = NULL;
  if (reInp) {
    SECURITY_ATTRIBUTES sa;
    sa.nLength = sizeof(sa);
    ...
    hOutput =
        CreateFileA("CONOUT$", GENERIC_WRITE | GENERIC_READ, FILE_SHARE_WRITE,
                    &sa, OPEN_EXISTING, /*FILE_ATTRIBUTE_NORMAL*/ 0, NULL);
  ...
  printf("\n--------------------------------");
  printf("\nProcess exited after %.4g seconds with return value %lu (%.4g ms "
         "cpu time, %lld KB mem used).\n",
