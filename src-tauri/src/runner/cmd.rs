...
pub fn parse_command(command: &str, _env: &HashMap<String, String>) -> Result<Command, String> {
	let (_, parts) = command_parser(command)
					...
					segments.into_iter().map(|segment| {
						if segment.starts_with('%') {
							...
							env.get(var_name)
								.cloned()
								.unwrap_or_else(|| {
									log::warn!("Environment variable '{}' not found, keeping original", var_name);
									segment.clone()
								})
						} else {
							segment
...
pub fn parse_command_with_env(command: &str, env: &HashMap<String, String>) -> Result<Command, String> {
	let (_, parts) = command_parser(command)
		.map_err(|e| format!("Failed to parse command: {:?}", e))?;
...
mod tests {
	...
	fn test_command_with_env_var() {
		...
		assert_eq!(result.get_program(), "ls");
		assert_eq!(result.get_args().collect::<Vec<_>>(), vec!["/home/user/test"]);
	}
	...
	fn test_mixed_quotes_and_env() {
		...
		assert_eq!(result.get_program(), "echo");
		assert_eq!(result.get_args().collect::<Vec<_>>(), vec!["Hello", "john", "World"]);
	}
	...
	fn test_mixed_escapes() {
		...
		assert_eq!(result.get_program(), "echo");
		assert_eq!(result.get_args().collect::<Vec<_>>(), vec!["Hello\"World\\Test"]);
	}
	...
	fn test_multiple_env_vars() {
		...
		assert_eq!(result.get_program(), "ls");
		assert_eq!(result.get_args().collect::<Vec<_>>(), vec!["/home/user/john"]);
	}
	...
	fn test_unclosed_quote() {
		...
		assert!(result.is_ok(), "nom parser returns error at end; we trim in parser");
	}
}